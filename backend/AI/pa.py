...
def get_database_data(ppa_filter=None, committee_filter=None, category_filter=None):
    """Fetches data from the database by executing the [Raw Data] stored procedure with optional filters."""
    # Convert string 'None' to Python None for proper SQL NULL parameter passing
    if ppa_filter == 'None': ppa_filter = None
    if committee_filter == 'None': committee_filter = None
    if category_filter == 'None': category_filter = None

    logger.info("Step 1/6: Connecting to database...")
    ...
    try:
        with pyodbc.connect(conn_str) as conn:
            ...
            # Updated to call the stored procedure with parameters
            query = "EXEC [Raw Data] @ppaFilter=?, @committeeFilter=?, @categoryFilter=?"
            
            # Execute the stored procedure with parameters
            df = pd.read_sql(query, conn, params=[ppa_filter, committee_filter, category_filter])
            
            logger.info(f"Step 1/6: Successfully fetched {len(df)} rows from the database.")
...
def main():
    ...
    try:
        raw_df = get_database_data(category_filter=category)
        
        if raw_df.empty:
            # If no data, return a specific JSON structure
            response = {
                "error": True,
