...
def get_database_data(category_filter=None):
    """Fetches and processes data from the [Raw Data] stored procedure."""
    if not all([DB_SERVER, DB_DATABASE, DB_USER, DB_PASSWORD]):
        raise ConnectionError("Database credentials are not fully configured.")
    
    conn_str = f'DRIVER={DB_DRIVER};SERVER={DB_SERVER};DATABASE={DB_DATABASE};UID={DB_USER};PWD={DB_PASSWORD}'
    try:
        with pyodbc.connect(conn_str) as conn:
            query = "EXEC [Raw Data] @ppaFilter=NULL, @committeeFilter=NULL, @categoryFilter=?"
            df = pd.read_sql(query, conn, params=[category_filter])
...
def main(custom_category, other_category, forecast_year):
    try:
        ...
        primary_data = get_database_data(category_filter=db_category_filter)
