...
router.post('/', upload, async (req, res) => {
    const { username, fullName, barangay, position, emailAddress, phoneNumber, dateOfBirth, password } = req.body;
    ...
    try {
        ...
        // 5. Call the FastAPI service via Axios
        const AI_SERVICE_URL = process.env.AI_SERVICE_URL || 'http://ai-service:8000';
        axios.post(`${AI_SERVICE_URL}/verify-registration`, { user_id: newUserId })
            .then(response => {
                console.log(`[AI_SERVICE] Response for userID ${newUserId}:`, response.data);
                handleAIJobCompletion(newUserId);
            })
            .catch(error => {
                console.error(`[AI_SERVICE] Error for userID ${newUserId}:`, error.message);
                // Fallback attempt to complete the job even if communication fails
                handleAIJobCompletion(newUserId);
            });
